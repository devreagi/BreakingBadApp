...
import {ItemButton, ItemContainer, ItemContent, ItemText, ItemTitle} from './styles/ItemCharacter.style';
    ...
    return (
        <ItemContainer className={toggleActive ? 'active' : ''} ref={itemRef}>
            ...
            <ItemContent>
                ...
                <hr/>
                <ItemText>
                    <p><span>Apodo:</span> {item.nickname}</p>
                    <p>
                        <span>Ocupación:</span> {item.occupation[0]} {item.occupation[1] ? `| ` + `${item.occupation[1]}` : ''}
                    </p>
